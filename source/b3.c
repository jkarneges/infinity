...
struct BOSSDATA bossdata[] = {
    ...
    {{0x0000, 0x0057, 0x2529, 0x0000, 0x0000, 0x0d9e, 0x56d1, 0x0000}, BOSS_COMPACT}, // yeti
...
BYTE get_player_input(UBYTE guy)
{
    ...
    if (binputmode == 0) {
        ...
        if (key[CANCEL] || !bch[bchoosetarget].active ||
            (bch[bchoosetarget].st.hp == 0 && bch[bchoosetarget].control == BCPU)) {
            wipe_text();
            // bmenu = 6;
            // bmenusquish = 16;
            binputmode = 1;
