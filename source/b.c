        ...
        if (inturn && !moved) {
            ...
            if (req != -1) {
                ...
                // attack
                    ...
                    // verify that the move is allowed
                    if (bch[b_target].x == dx && bch[b_target].y == dy && bch[b_target].active &&
                        bch[b_target].st.hp > 0) {
                        d = f_get_4waydir(dir);
                ...
                // skill (30 skills, 16 gems, 12 items (66-77)
                    ...
                    b_target = bch[turn].target;
                    if (bch[b_target].active &&
                        !(bch[b_target].st.hp <= 0 && bch[b_target].control != bch[turn].control)) {
                        n = req - 16;
                        // n = bch[turn].skl[n] - 1;
