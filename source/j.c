...
BYTE mrel[8] = {0, -1, 0, 1, -1, 0, 1, 0};

BYTE mrel2[8] = {0, -MOVESPEED, 0, MOVESPEED, -MOVESPEED, 0, MOVESPEED, 0};
        ...
        while (game_mode == 1) {
            ...
            if (!in_event && ch[0].mode == NORM) {
                ...
                if (!in_event) {
                    ...
                    // check for ice
                    for (n = 0;; ++n) {
                        if (!owe_blocked(3, b1 >> 1, b2 >> 1) || !owe_blocked(3, (b1 + 1) >> 1, b2 >> 1) ||
                            owe_clip(b1, b2, ch[0].dir) != ch[0].dir + 1)
                            break;
                        b1 += mrel[ch[0].dir << 1];
                    ...
                    if (map_cur == 0) {
                        if (owe_blocked(0, ch[0].x >> 1, (ch[0].y + 1) >> 1) &&
                            owe_blocked(0, (ch[0].x + 1) >> 1, (ch[0].y + 1) >> 1))
                            init_event(0x0024);
                    }
                }
            }
                            ...
                            if (n) {
                                ...
                                if (!map_cur) {
                                    if (!on_boat) {
                                        ...
                                        n = owe_clip(ch[0].x + (mrel[ch[0].dir << 1] << 1),
                                                     ch[0].y + mrel[(ch[0].dir << 1) + 1] + 1, ch[0].dir);
                                        if (n - 1 == ch[0].dir) {
